 ...
 * @param callback
 */
Table.prototype.update = function(values,condition ,callback) {
    if(!callback){
        callback=function(){};
    }
    var me=this;
    ...
    for (var pro in condition){
         sql += " and " + pro + "=" +this.pool.escape(condition[pro]);
    }
    ...
    if (this.clearTable(values)) {
        this.getConnection(function(connection) {
            var query = connection.query("update  " + me.tablename + " set ? where 1=1 "+sql, values, function(err, result) {
                if (err) {
                    callback(err,result);
                }else{
 ...
 * @param callback
 */
Table.prototype.query = function(params,orders, callback) {
    if(!callback){
        callback=function(){};
    }
    var sql = "select  ";
